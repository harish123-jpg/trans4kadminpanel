...
def generate_script(payload):
    ...
    is_cpu_transcode = is_cpu_transcode = force_cpu or gpu_value in ['', 'none', 'null', '0']
    if output_stream == "HLS":
...
def get_bandwidth_data(request):
    ...
    for stream in Stream.objects.all():
        ch = stream.name
        udp_url = stream.source  # udp://IP:PORT

        last_time = LAST_READ_TIME.get(ch, now)
        dt = max(now - last_time, 1)

        # ------------------------------------------
        # 1️⃣ INPUT BANDWIDTH (UDP → FFmpeg reader)
        # ------------------------------------------
        input_mbps = 0
        try:
            bytes_in = 0
            ...
            if ip_port in str(udp_ports):
                # Fast: scan only processes containing udp_url
                for p in psutil.process_iter(['pid', 'cmdline']):
                    try:
                        cmd = " ".join(p.info['cmdline']) if p.info['cmdline'] else ""
                        if udp_url in cmd:
        ...
        # ------------------------------------------
        # 2️⃣ OUTPUT BANDWIDTH (HLS: last 3 segments)
        # ------------------------------------------
        output_mbps = 0
        try:
            ch_files = [f for f in all_files if os.path.basename(f).startswith(ch)]
            ch_files = sorted(ch_files, key=os.path.getmtime)[-3:]  # last 3 segments only
            ...
            if len(ch_files) >= 2:
                ...
                output_mbps = (size * 2 * 8 / 4) / 1e6  # approximate fallback
